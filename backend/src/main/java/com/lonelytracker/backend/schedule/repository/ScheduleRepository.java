...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
     ...
     * </ol>
     * 정확한 날짜 판정은 {@link OccurrenceExpander} 가 한다. 여기서는 후보만 좁힌다.
     */
    @Query("""
            ...
            where s.user.id = :userId
              and s.startAt <= :to
              and (
                   exists (select 1 from ScheduleRecur r
                            where r.scheduleId = s.id
                              and (r.endsOn is null or r.endsOn >= :fromDate))
                or s.startAt >= :from
                or exists (select 1 from ScheduleProgress p
            ...
            """)
    List<Schedule> findCandidates(@Param("userId") Long userId,
