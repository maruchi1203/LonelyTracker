 ...
 * <p>
 * 손댄 회차만 {@link ScheduleProgress} 행이 된다.
...
public class ScheduleOccurrenceService {
    ...
    public ScheduleResponse changeStatus(Long scheduleId, LocalDate onDate, ScheduleStatus status) {
        ScheduleProgress progress = getOrCreate(scheduleId, onDate);
     ...
     */
    private ScheduleProgress getOrCreate(Long scheduleId, LocalDate onDate) {
        Schedule schedule = scheduleService.getOwnedOrThrow(scheduleId);
        ...
        return progressRepository.findByScheduleIdAndOnDate(scheduleId, onDate)
                .orElseGet(() -> ScheduleProgress.builder()
                        .schedule(schedule)
                        .onDate(onDate)
     ...
     * 엉뚱한 회차가 반환된다.
     */
    private ScheduleResponse toResponse(ScheduleProgress progress) {
        LocalDate onDate = progress.getOnDate();
        Schedule schedule = progress.getSchedule();
